...
### Rivals (John)
- [ ] [rivals/01-basic-bot](features/rivals/01-foundation/): `BotController` deciding every 0.3 s (value ÷ distance target; bank when greedy or time is short; opportunistic ram), `NavigationAgent3D` pathing (waypoint fallback), 1 s unstick, test scene with dummy pickups
