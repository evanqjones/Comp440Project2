...
- **Handoff notes:**
  ...
  - **player/09-title (D-029):** `TitleFlow` runs title (store name, Grandma's card, "press any key", which also satisfies the web-audio rule) → story → "Meet your rivals" (Carl, Bev and Rita's ID cards and blurbs) → `RoundManager.start_match()`. `ShopperIdCard.make(profile)` fills Evan's `assets/ui/id_card_layout.tscn` **automatically once it exists** (placeholder with the same `%` names until then). Restarts skip the intro; the HUD hides while IDLE. **GUT: 21 scripts, 164/164.** Checked with Run Project: screenshots of all three screens; the countdown starts after the last key.
  - **Anthony (start):** your RoundManager's `start_match()` should start the match; `TitleFlow` calls it (see D-029). Until then, the demo stand-in does.
...
- **In progress:** nothing. Next for Final: match results + card reissue (needs Anthony's best of 3), audio (needs Evan's files), slip (with Anthony's wet floor), character select (Q-004).
