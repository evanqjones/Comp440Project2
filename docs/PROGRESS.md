...
- **Done:** `Step 1.1` (BotPersonality resource script), `Step 1.2` (base BotController and round timer wiring), `Step 2.1` (COLLECTING state utility scoring math), `Step 2.2` (BANKING state transitions), `Step 3.1` (CHASING state target selection and aggression gates), and `Step 3.2` (Additive aggression scaling per round) implemented and tested (94/94 tests passing, 0 script errors).
- **In progress:** Step 4.1: Stuck detection and recovery steering.
- **Next:** Implement FSM stuck recovery, navigation blacklists, periodic boost gates, and events.
